...
public extension KodableOption {
    ...
    static var enforceType: KodableOption {
        .propertyDecoding(.enforceType)
    }
    ...
    static var lossless: KodableOption {
        .propertyDecoding(.lossless)
    }
    ...
    static var lossy: KodableOption {
        .propertyDecoding(.lossy)
    }
}
...
public extension KodableOption {
    static func overrideValue(_ overrideValue: @escaping KodableModifier<Value>.OverrideValueClosure) -> KodableOption {
        .modifier(KodableModifier(overrideValue))
    }

    static func validation(_ validation: @escaping KodableModifier<Value>.ValidationClosure) -> KodableOption {
        .modifier(KodableModifier(validation))
    }
}
...
public extension KodableOption where Value == String {
    ...
    static var trimmed: KodableOption {
        overrideValue { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
    }
}
...
public extension KodableOption where Value: Comparable {
    ...
    static func clamping(to range: ClosedRange<Value>) -> KodableOption {
        overrideValue { $0.constrained(to: range) }
    }

    /// Constrains the value inside a `range`. Only applicable to types conforming to Comparable
    static func range(_ range: ClosedRange<Value>) -> KodableOption {
        overrideValue { $0.constrained(to: range) }
    }

    /// Constrains the value to a `max` value. Only applicable to types conforming to Comparable
    static func max(_ value: Value) -> KodableOption {
        overrideValue { $0.constrained(toAtMost: value) }
    }

    /// Constrains the value to a `min` value. Only applicable to types conforming to Comparable
    static func min(_ value: Value) -> KodableOption {
        overrideValue { $0.constrained(toAtLeast: value) }
    }
}
...
public extension KodableOption where Value: Collection, Value.Element: Comparable {
    ...
    static var ascending: KodableOption {
        sorted { $0 < $1 }
    }

    /// Sorts the elements of an array descending
    static var descending: KodableOption {
        sorted { $0 > $1 }
    }
}
